...
import { LeadScoreBadge } from "./crm-features/LeadScoreBadge";
import { PriorityTagStrip } from "./crm-features/PriorityTagStrip";
import { WhatsAppGenerator } from "./crm-features/WhatsAppGenerator";
import { LostReasonModal } from "./crm-features/LostReasonModal";
import { PropertyRecommendation } from "./crm-features/PropertyRecommendation";
import { TokenBookingPanel } from "./crm-features/TokenTracker";
import {
  ...
  Building2, Video, Briefcase, XCircle,
} from "lucide-react";
...
export function LeadControlPanel() {
  ...
  // FEATURE 6: Lost reason modal state
      ...
      <SheetContent side="right" className="w-full sm:max-w-[560px] p-0 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          <Tabs value={tab} onValueChange={setTab} className="px-5 py-4">
            ...
            {/* ROOMS — Smart Property Recommendation */}
            <TabsContent value="rooms" className="space-y-4 pt-4">
              <Section title="Smart property recommendations">
                <p className="text-[11px] text-muted-foreground">
                  Top matches based on budget ₹{lead.budget?.toLocaleString("en-IN") ?? "—"} · area: {lead.preferredArea ?? "—"}
                </p>
                <PropertyRecommendation lead={lead} />
              </Section>
            </TabsContent>
            ...
            <TabsContent value="control" className="space-y-4 pt-4">
              ...
              <Section title="Status engine">
                <Select value={lead.stage} onValueChange={(v) => {
                  ...
                  if (v === "dropped") {
                    // FEATURE 6: Show lost reason modal instead of direct drop
                ...
                {/* FEATURE 6: Quick "Mark as Lost" CTA */}
                {lead.stage !== "dropped" && lead.stage !== "booked" && (
                  <Button
                    size="sm"
                    variant="outline"
                    className="w-full h-8 text-destructive border-destructive/30 hover:bg-destructive/10 gap-1.5"
              ...
              <Section title="Action engine">
                ...
                <WhatsAppGenerator lead={lead} className="w-full" />
            ...
            {/* TOKEN — Payment & booking tracker */}
            <TabsContent value="token" className="space-y-4 pt-4">
              <Section title="Token & payment tracking">
                <TokenBookingPanel lead={lead} />
              </Section>
            </TabsContent>
