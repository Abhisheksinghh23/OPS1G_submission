...
interface AppState {
  ...
  // FEATURE 6: mark lead lost with a reason
  markLeadLost: (leadId: string, reason: string) => void;
  // FEATURE 8: mark lead as duplicate of another
  markAsDuplicate: (leadId: string, originalId: string) => void;
}
...
export const useApp = create<AppState>((set, get) => ({
  ...
  addLead: (input) => {
    ...
    // FEATURE 8: Duplicate detection by phone number
    const existingByPhone = get().leads.find(
      (l) => l.phone.replace(/\s/g, "") === input.phone.replace(/\s/g, "") && l.stage !== "dropped",
    );
    if (existingByPhone) {
      // Log a duplicate activity instead of creating a new lead
      pushActivity(set, get, {
        kind: "status_changed", actor: "flow-ops", leadId: existingByPhone.id,
        text: `Duplicate enquiry received via ${input.source ?? "Public Landing Page"} — existing lead updated`,
      });
      // Update the existing lead's updatedAt so it surfaces in the feed
      set((s) => ({
        leads: s.leads.map((l) =>
          l.id === existingByPhone.id
            ? { ...l, updatedAt: new Date().toISOString(), isDuplicate: false }
            : l,
        ),
      }));
      return;
  ...
  // FEATURE 6: Mark lead lost with reason
  markLeadLost: (leadId, reason) => {
    set((s) => ({
      leads: s.leads.map((l) =>
        l.id === leadId
          ? {
              ...
              lostReason: reason,
              updatedAt: new Date().toISOString(),
              tags: [...l.tags.filter((t) => !t.startsWith("lost:")), `lost:${reason}`],
            }
          : l,
      ),
    }));
    pushActivity(set, get, {
      kind: "status_changed", actor: get().role, leadId,
      text: `Lead marked lost · reason: ${reason}`,
    });
  },

  // FEATURE 8: Mark as duplicate
  markAsDuplicate: (leadId, originalId) => {
    set((s) => ({
      leads: s.leads.map((l) =>
        l.id === leadId
          ? { ...l, isDuplicate: true, duplicateOfId: originalId, updatedAt: new Date().toISOString() }
          : l,
      ),
    }));
    pushActivity(set, get, {
      kind: "status_changed", actor: get().role, leadId,
      text: `Marked as duplicate of lead #${originalId}`,
    });
  },
}));
