...
export function bestInventoryFits(input: {
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed?.matching) {
          ...
          if (typeof m.wAvailability === "number") wAvailability = m.wAvailability;
          if (typeof m.wConversion === "number") wConversion = m.wConversion;
  ...
  let matches = matchLead(supplyLead);

  // Apply Settings Toggles
  if (showOnlyVerified) {
    matches = matches.filter((m) => m.pg.iqBreakdown?.["GPS Verified"]?.ok);
  }
  if (hideLowCompliance) {
    matches = matches.filter((m) => m.pg.iq >= 75);
  }
  ...
  return matches
    ...
    .map((m) => {
      ...
      // Dynamically compute Availability score component out of wAvailability
      const availabilityPts = Math.round((Math.min(5, inv.beds) / 5) * wAvailability);
      // Dynamically compute Conversion score component out of wConversion (approximated from IQ)
      const conversionPts = Math.round((p.iq / 100) * wConversion);

      const score = Math.max(0, Math.round(m.total + availabilityPts + conversionPts));
      return {
        propertyId: p.id,
        propertyName: p.name,
