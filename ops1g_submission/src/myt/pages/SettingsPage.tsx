    ...
    <div className="mx-auto max-w-6xl space-y-4">
      ...
      <Tabs defaultValue="matching" className="space-y-4">
        ...
        <TabsContent value="automation" className="space-y-3">
          <AutomationEditor rules={settings.automation} onChange={(v) => update("automation", v)} />
        </TabsContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            />
            <SwitchRow
              label="Show only verified properties"
              hint="Excludes properties that are not GPS verified."
              checked={matching.showOnlyVerified}
              onCheckedChange={(checked) => onChange({ ...matching, showOnlyVerified: checked })}
            />
            <SwitchRow
              label="Hide low compliance properties"
              hint="Excludes properties with compliance score below 75."
              checked={matching.hideLowCompliance}
              onCheckedChange={(checked) => onChange({ ...matching, hideLowCompliance: checked })}
            />
          </div>
        </CardContent>
      </Card>
...
function AutomationEditor({ rules, onChange }: { rules: any[]; onChange: (v: any[]) => void }) {
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2">
          <Settings2 className="h-4 w-4 text-accent" /> System Automation Rules
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          Activate or deactivate core operational routing logic and triggers.
        </p>
      </CardHeader>
      <CardContent className="grid gap-3 sm:grid-cols-2 p-4">
        {rules.map((rule) => (
          <div key={rule.id} className="flex items-start justify-between gap-3 rounded-lg border border-border bg-muted/20 p-3">
            <div>
